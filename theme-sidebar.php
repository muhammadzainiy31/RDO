<!--**********************************
    Sidebar start
***********************************-->
<div class="deznav">
    <div class="deznav-scroll">
        <ul class="metismenu" id="menu">
            <li>
                <a class="has-arrow ai-icon" href="javascript:void()" aria-expanded="false">
                    <i class="flaticon-381-networking"></i>
                    <span class="nav-text">DASHBOARD</span>
                </a>
                <ul aria-expanded="false">
                    <li><a href="../dashboard/index.php">Dashboard</a></li>
                </ul>
            </li>
            <li>
                <a class="has-arrow ai-icon" href="javascript:void()" aria-expanded="false">
                    <i class="flaticon-381-network"></i>
                    <span class="nav-text">DATA</span>
                </a>
                <ul aria-expanded="false">
                    <li><a href="../surat/index.php">Surat Penjualan</a></li>
                    <li><a href="../pengiriman/index.php">Pengiriman</a></li>
                    <li><a href="../customer/index.php">Customer</a></li>
                    <li><a href="../barang/index.php">Barang</a></li>
                    <li><a href="../departemen/index.php">Departemen</a></li>
                    <li><a href="../driver/index.php">Driver</a></li>
                    <li><a href="../armada/index.php">Armada</a></li>
                    <li><a href="../admin/index.php">Admin</a></li>
                </ul>
            </li>
            <li>
                <a class="has-arrow ai-icon" href="javascript:void()" aria-expanded="false">
                    <i class="flaticon-381-notepad"></i>
                    <span class="nav-text">INPUT</span>
                </a>
                <ul aria-expanded="false">
                    <li><a href="../surat/input.php">Surat</a></li>
                    <li><a href="../customer/input.php">Customer</a></li>
                    <li><a href="../barang/input.php">Barang</a></li>
                    <li><a href="../driver/input.php">Driver</a></li>
                    <li><a href="../armada/input.php">Armada</a></li>
                    <li><a href="../lap_riw_servis_mobil/input.php">Servis Armada</a></li>
                    <li><a href="../departemen/input.php"> Departemen</a></li>
                    <li><a href="../admin/input.php">Admin </a></li>
                </ul>
            </li>
            <li>
            <li>
                <a class="has-arrow ai-icon" href="javascript:void()" aria-expanded="false">
                    <i class="flaticon-381-network"></i>
                    <span class="nav-text">LAPORAN</span>
                </a>
                <ul aria-expanded="false">
                    <li><a href="../pengiriman/index.php">Pengiriman</a></li>
                    <li><a href="../lap_surat_jalan/index.php">Surat Jalan</a></li>
                    <li><a href="../lap_minim_barang_stok/index.php">Barang Minim stok</a></li>
                    <li><a href="../lap_barang_laris/index.php">Barang Terlaris</a></li>
                    <li><a href="../riwayat_lap_barang_laris/index.php">Riwayat Barang Terlaris</a></li>
                    <li><a href="../lap_berlaku_sim/index.php">Berlaku SIM Driver</a></li>
                    <li><a href="../lap_cust_berdasarkan_wil/index.php">Customer berdasarkan Kecamatan</a></li>
                    <li><a href="../rekap_cust_berdasarkan_wil/index.php">Rekap Customer berdasarkan Kecamatan</a></li>
                    <li><a href="../lap_cust_royal/index.php">Customer Paling Royal</a></li>
                    <li><a href="../lap_riw_pengiriman_driver/index.php">Riwayat Pengiriman Driver</a></li>
                    <li><a href="../rekap_pengiriman_driver/index.php">Rekap Pengiriman Driver</a></li>
                    <li><a href="../lap_jarak_tempuh/index.php">Jarak Tempuh Armada</a></li>
                    <li><a href="../lap_riw_servis_mobil/index.php">Riwayat Servis Armada</a></li>
                    <li><a href="../rdo/index.php">RDO</a></li>
                </ul>
            </li>
        </ul>
    </div>
</div>
<!--**********************************
    Sidebar end
***********************************-->